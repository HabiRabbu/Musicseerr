...
function createDiscoverQueueStatusStore() {
	...
	async function fetchStatus(source?: MusicSource): Promise<QueueStatusPayload | null> {
		...
		try {
			...
			const res = await fetch(API.discoverQueueStatus(activeSource));
			if (!res.ok) return null;
			const data: QueueStatusPayload = await res.json();
	...
	async function triggerGenerate(force = false, source?: MusicSource): Promise<void> {
		...
		try {
			...
			const res = await fetch(API.discoverQueueGenerate(), {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ source: activeSource, force }),
			});
			if (res.ok) {
				const data: QueueStatusPayload = await res.json();
				applyStatusData(data);
