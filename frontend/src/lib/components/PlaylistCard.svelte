<script lang="ts">
	...
	async function handlePlay(e: Event) {
		...
		try {
			...
			toastStore.show({ message: 'Failed to load playlist', type: 'error' });
		} finally {
			playLoading = false;
		}
	}
...
<div
	class="card card-sm bg-base-100 w-full shadow-sm flex-shrink-0 group relative transition-shadow hover:shadow-lg"
>
	<a
