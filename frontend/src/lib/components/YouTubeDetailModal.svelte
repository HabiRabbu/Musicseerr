<script lang="ts">
	import { Shuffle, Pencil, Trash2, Play, X } from 'lucide-svelte';
						...
						{#each tracks as track}
							...
							>
								...
								<span style={isCurrentlyPlaying ? `color: ${colors.accent};` : ''}>
									<Play class="h-4 w-4 flex-shrink-0 transition-opacity {isCurrentlyPlaying ? 'opacity-100' : 'text-accent opacity-0 group-hover/track:opacity-100'} fill-current" />
								</span>
							</button>
						{/each}
					</div>
