<script lang="ts">
	...
	function handleSearchInput(): void {
		...
		searchTimeout = setTimeout(async () => {
			try {
				const res = await fetch(API.search.albums(searchQuery.trim()));
				if (res.ok) {
	...
	async function handleSave(): Promise<void> {
		...
		try {
			if (isEditing && selectedAlbumId) {
				const res = await fetch(API.youtube.updateLink(selectedAlbumId), {
					method: 'PUT',
				...
				const updated: YouTubeLink = await res.json();
				onsave(updated);
				toastStore.show({ message: 'Link updated', type: 'success', duration: TOAST_DURATION });
			} else {
				const res = await fetch(API.youtube.manual(), {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({
						album_name: albumName.trim(),
				...
				await throwOnApiError(res, 'Failed to save');
				const link: YouTubeLink = await res.json();
				onsave(link);
			...
			toastStore.show({
				message: e instanceof Error ? e.message : 'Failed to save link',
				type: 'error',
