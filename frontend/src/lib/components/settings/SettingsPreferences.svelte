	...
	async function loadLidarrPrefs() {
		...
		try {
			if (lidarrProfiles.length === 0) {
				const profilesRes = await fetch('/api/settings/lidarr/metadata-profiles');
				if (profilesRes.ok) {
			...
			const response = await fetch(
				`/api/settings/lidarr/metadata-profile/preferences${params}`
			);
			if (response.ok) {
		...
		try {
			...
			if (response.ok) {
				...
				try {
					...
					lidarrMessage = err.detail || 'Failed to update Lidarr metadata profile';
				} catch {
