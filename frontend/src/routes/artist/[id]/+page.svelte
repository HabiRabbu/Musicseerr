<script lang="ts">
	...
	function onHeroImageLoad() {
		...
		// Only extract dominant color for gradient if no fanart available
		if (artist && (!artist.fanart_url || fanartError)) {
	...
	// Extract dominant color when fanart fails (for gradient fallback)
	...
	{:else if artist}
		<div class="space-y-4 sm:space-y-6 lg:space-y-8">
			...
			<div class="relative -mx-2 sm:-mx-4 lg:-mx-8 -mt-4 sm:-mt-8 overflow-hidden">
				<!-- Fanart background (if available) -->
				{#if artist.fanart_url && !fanartError}
					<div class="absolute inset-0">
						<img
							src={artist.fanart_url}
							alt=""
							class="w-full h-full object-cover transition-opacity duration-700 {fanartLoaded ? 'opacity-30' : 'opacity-0'}"
							loading="eager"
							on:load={() => fanartLoaded = true}
							on:error={() => fanartError = true}
						/>
						<div class="absolute inset-0 bg-gradient-to-b from-transparent via-base-100/60 to-base-100"></div>
					</div>
				{:else}
					<!-- Fallback gradient from dominant color -->
					...
					<div class="max-w-7xl mx-auto">
						<div class="flex flex-col sm:flex-row items-center sm:items-end gap-6 sm:gap-8">
							<div class="flex-shrink-0">
								<div class="relative">
									<div class="w-40 h-40 sm:w-52 sm:h-52 lg:w-64 lg:h-64 rounded-full overflow-hidden shadow-2xl ring-4 ring-base-100/20" style="background-color: #0d120a;">
								...
								<div class="list-row group hover:bg-base-300 transition-colors p-0" role="listitem">
									...
									<div class="flex items-center flex-shrink-0 ml-auto mr-3 sm:mr-4">
										...
										{:else if rg.requested}
											<div class="w-8 h-8 sm:w-10 sm:h-10 rounded-full shadow-sm flex items-center justify-center" style="background-color: #F59E0B;">
												<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke={colors.secondary} stroke-width="2">
