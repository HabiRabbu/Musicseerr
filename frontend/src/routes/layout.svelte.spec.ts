...
vi.mock('$lib/stores/integration', () => ({
	integrationStore: {
		subscribe: vi.fn((cb: (v: unknown) => void) => {
			cb({ lidarr: false, jellyfin: false, listenbrainz: false, youtube: false, localfiles: false, lastfm: false, loaded: true });
			return () => {};
		}),
...
describe('+layout.svelte sidebar', () => {
	...
	it('renders "Playlists" link in the sidebar', async () => {
		renderLayout();
