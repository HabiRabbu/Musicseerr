<script lang="ts">
	...
	async function saveName() {
		...
		try {
			...
			playlist.updated_at = updated.updated_at;
			editingName = false;
			onplaylistupdate(playlist);
			toastStore.show({ message: 'Playlist renamed', type: 'success' });
		} catch {
			toastStore.show({ message: 'Failed to rename playlist', type: 'error' });
		} finally {
			savingName = false;
		}
	}
	...
	async function handleCoverSelect(e: Event) {
		...
		const prevCover = playlist.custom_cover_url;
		coverPreview = URL.createObjectURL(file);
		uploading = true;
		try {
			const result = await uploadPlaylistCover(playlist.id, file);
			playlist.custom_cover_url = result.cover_url + '?t=' + Date.now();
			onplaylistupdate(playlist);
			toastStore.show({ message: 'Cover updated', type: 'success' });
		} catch {
			playlist.custom_cover_url = prevCover;
			toastStore.show({ message: 'Failed to upload cover', type: 'error' });
		} finally {
			if (coverPreview) {
	...
	async function removeCover() {
		const prevCover = playlist.custom_cover_url;
		playlist.custom_cover_url = null;
		try {
			...
			playlist.custom_cover_url = prevCover;
			toastStore.show({ message: 'Failed to remove cover', type: 'error' });
		}
	}
