...
from core.dependencies import get_playlist_service, get_jellyfin_library_service, get_local_files_service
...
@pytest.fixture
def client(mock_playlist_service, mock_jf_service, mock_local_service):
    app = FastAPI()
...
class TestGetPlaylist:
    def test_success(self, client):
        resp = client.get("/api/v1/playlists/p-1")
        assert resp.status_code == 200
        data = resp.json()
        assert data["id"] == "p-1"
    ...
    def test_not_found(self, client, mock_playlist_service):
        ...
        resp = client.get("/api/v1/playlists/nonexistent")
...
class TestDeletePlaylist:
    def test_success(self, client):
        resp = client.delete("/api/v1/playlists/p-1")
        assert resp.status_code == 200
        assert resp.json()["status"] == "ok"

    def test_not_found(self, client, mock_playlist_service):
        mock_playlist_service.delete_playlist.side_effect = PlaylistNotFoundError("nope")
        resp = client.delete("/api/v1/playlists/nonexistent")
        assert resp.status_code == 404
...
class TestAddTracks:
    def test_success(self, client):
        body = {
            "tracks": [
                {"track_name": "S", "artist_name": "A", "album_name": "AL"},
            ],
        }
        resp = client.post(
            "/api/v1/playlists/p-1/tracks",
    ...
    def test_playlist_not_found(self, client, mock_playlist_service):
        ...
        resp = client.post(
            "/api/v1/playlists/nonexistent/tracks",
            content=__import__("json").dumps(body).encode(),
            headers={"Content-Type": "application/json"},
        )
        assert resp.status_code == 404
...
class TestRemoveTrack:
    def test_success(self, client):
        resp = client.delete("/api/v1/playlists/p-1/tracks/t-1")
        assert resp.status_code == 200
        assert resp.json()["status"] == "ok"

    def test_not_found(self, client, mock_playlist_service):
        mock_playlist_service.remove_track.side_effect = PlaylistNotFoundError("nope")
        resp = client.delete("/api/v1/playlists/p-1/tracks/nonexistent")
        assert resp.status_code == 404
...
class TestReorderTrack:
    def test_success(self, client):
        resp = client.patch(
            "/api/v1/playlists/p-1/tracks/reorder",
...
class TestUpdateTrack:
    ...
    def test_not_found(self, client, mock_playlist_service):
        ...
        resp = client.patch(
            "/api/v1/playlists/p-1/tracks/nonexistent",
            content=b'{"source_type": "youtube"}',
            headers={"Content-Type": "application/json"},
        )
        assert resp.status_code == 404
...
class TestGetCover:
    def test_no_cover(self, client):
        resp = client.get("/api/v1/playlists/p-1/cover")
        assert resp.status_code == 404
...
class TestRemoveCover:
    def test_success(self, client):
        resp = client.delete("/api/v1/playlists/p-1/cover")
        assert resp.status_code == 200
        assert resp.json()["status"] == "ok"
...
class TestResolveSources:
    def test_success(self, client, mock_playlist_service):
        ...
        resp = client.post("/api/v1/playlists/p-1/resolve-sources")
        assert resp.status_code == 200
        data = resp.json()
