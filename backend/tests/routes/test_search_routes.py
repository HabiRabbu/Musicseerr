...
def test_suggest_accepts_two_char_query(client, mock_search_service):
    response = client.get("/api/search/suggest?q=ab")
...
def test_suggest_limit_lower_bound(client, mock_search_service):
    response = client.get("/api/search/suggest?q=test&limit=0")

    assert response.status_code == 422


def test_suggest_limit_upper_bound(client, mock_search_service):
    response = client.get("/api/search/suggest?q=test&limit=11")
...
def test_suggest_custom_limit(client, mock_search_service):
    response = client.get("/api/search/suggest?q=test&limit=3")
...
def test_suggest_whitespace_padded_short_input_returns_empty(client, mock_search_service):
    ...
    response = client.get("/api/search/suggest?q=%20%20a%20%20")
...
def test_suggest_whitespace_padded_valid_input_strips(client, mock_search_service):
    ...
    response = client.get("/api/search/suggest?q=%20%20ab%20%20")
...
def test_suggest_debug_log_contains_fields_without_query_text(client, mock_search_service, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="api.v1.routes.search"):
        response = client.get("/api/search/suggest?q=muse")
...
def test_search_response_tolerates_additive_score_field():
    ...
    response = search_client.get("/api/search?q=muse")
