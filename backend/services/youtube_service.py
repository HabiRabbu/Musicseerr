...
class YouTubeService:
    ...
    async def generate_track_link(
        ...
        video_id = await self._youtube_repo.search_video(track_name, artist_name)
        if not video_id:
            raise ResourceNotFoundError(
        ...
        await self._library_cache.save_youtube_track_link(
            album_id=album_id,
            album_name=album_name,
    ...
    async def get_track_links(self, album_id: str) -> list[YouTubeTrackLink]:
        results = await self._library_cache.get_youtube_track_links(album_id)
        return [YouTubeTrackLink(**row) for row in results]

    async def delete_track_link(self, album_id: str, track_number: int) -> None:
        await self._library_cache.delete_youtube_track_link(album_id, track_number)

    async def _save_and_return_link(self, **kwargs: object) -> YouTubeLink:
        await self._library_cache.save_youtube_link(**kwargs)
