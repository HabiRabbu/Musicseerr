...
class LastFmRepository:
    ...
    # --- User-data read methods (TTL 300s) ---
    ...
    async def get_user_top_artists(
        ...
        if period not in ALLOWED_LASTFM_PERIOD:
            period = "overall"
        cache_key = f"lfm_user_top_artists:{username}:{period}:{limit}"
    ...
    async def get_user_top_albums(
        ...
        if period not in ALLOWED_LASTFM_PERIOD:
            period = "overall"
        cache_key = f"lfm_user_top_albums:{username}:{period}:{limit}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        data = await self._request(
    ...
    async def get_user_top_tracks(
        ...
        if period not in ALLOWED_LASTFM_PERIOD:
            period = "overall"
        cache_key = f"lfm_user_top_tracks:{username}:{period}:{limit}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        data = await self._request(
    ...
    async def get_user_recent_tracks(
        ...
        cache_key = f"lfm_user_recent:{username}:{limit}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        data = await self._request(
    ...
    async def get_user_loved_tracks(
        ...
        cache_key = f"lfm_user_loved_tracks:{username}:{limit}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        data = await self._request(
    ...
    async def get_user_weekly_artist_chart(
        ...
        cache_key = f"lfm_user_weekly_artists:{username}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        data = await self._request(
    ...
    async def get_user_weekly_album_chart(
        ...
        cache_key = f"lfm_user_weekly_albums:{username}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        data = await self._request(
    ...
    # --- Entity-data read methods (TTL 3600s) ---

    async def get_artist_top_tracks(
        self, artist: str, mbid: str | None = None, limit: int = 10
    ) -> list[LastFmTrack]:
        lookup = mbid or artist
        cache_key = f"lfm_artist_top_tracks:{lookup}:{limit}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
    ...
    async def get_artist_top_albums(
        ...
        lookup = mbid or artist
        cache_key = f"lfm_artist_top_albums:{lookup}:{limit}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        params: dict[str, str] = {"limit": str(limit)}
    ...
    async def get_artist_info(
        ...
        lookup = mbid or artist
        cache_key = f"lfm_artist_info:{lookup}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        params: dict[str, str] = {}
    ...
    async def get_album_info(
        ...
        cache_key = f"lfm_album_info:{lookup}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        params: dict[str, str] = {}
    ...
    async def get_similar_artists(
        ...
        lookup = mbid or artist
        cache_key = f"lfm_similar_artists:{lookup}:{limit}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        params: dict[str, str] = {"limit": str(limit)}
    ...
    # --- Global/chart read methods (TTL 3600s) ---

    async def get_global_top_artists(self, limit: int = 50) -> list[LastFmArtist]:
        cache_key = f"lfm_global_top_artists:{limit}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        data = await self._request(
    ...
    async def get_global_top_tracks(self, limit: int = 50) -> list[LastFmTrack]:
        cache_key = f"lfm_global_top_tracks:{limit}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        data = await self._request(
    ...
    async def get_tag_top_artists(
        ...
        cache_key = f"lfm_tag_top_artists:{tag}:{limit}"
        cached = await self._cache.get(cache_key)
        if cached is not None:
            return cached
        data = await self._request(
